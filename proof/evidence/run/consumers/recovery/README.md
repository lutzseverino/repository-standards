...
The interface is `python3 app.py FILE COLUMN [--precision N]`; `python3 app.py --help` shows
...
```text
usage: app.py [-h] [--precision PRECISION] file column
app.py: error: could not convert string to float: 'oops'
```

By default, the total is the unrounded floating-point sum. `--precision N`
rounds that total to N decimal places using Python's `round()`; it does not
change input validation or guarantee trailing zeroes in the JSON output.
For example:

```sh
cat > /tmp/ledger-decimal.csv <<'CSV'
item,amount
Service,1.234
Refund,2.345
CSV
python3 app.py /tmp/ledger-decimal.csv amount
python3 app.py /tmp/ledger-decimal.csv amount --precision 2
```

Observed output, in command order:

```json
{"column": "amount", "count": 2, "total": 3.579}
{"column": "amount", "count": 2, "total": 3.58}
```
