...
| `ready-for-agent` | `ready-for-agent` | An agent can implement the work autonomously |
...
`ready-for-agent` expresses specification readiness. Selection and startup are
external dispatch actions; the label does not provide an automatic pickup
system or describe an execution state.
